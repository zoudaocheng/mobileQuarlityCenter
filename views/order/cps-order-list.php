...
<table id="" class="table table-striped table-bordered responsive">
    ...
    <?php foreach ($provider->models as $order) {?>
        <tr>
            ...
            <td class="center"><?= $order->order->sa['sa_name']?$order->order->sa['sa_name']:'无'?></td>
            <td class="center"><?= $order->order->sa['sa_mobile']?$order->order->sa['sa_mobile']:'无'?></td>
            <td class="center"><?php
                if(1 == $order->order->sa['source'])
                {
                    echo 'SA抢单';
                }elseif(2 == $order->order->sa['source'])
                {
                    echo '自动分配';
                }else{
                    echo '抢单关闭';
            ...
            <td class="center"><?= $order->order->orderCoupon['coupon_number']?$order->order->orderCoupon['coupon_number']:'无'?></td>
            <td class="center"><?php
                if(1 == $order->order->orderCoupon['used_status']){
                    echo '未使用';
                }elseif(2 == $order->order->orderCoupon['used_status']){
                    echo '已确认';
                }elseif(3 == $order->order->orderCoupon['used_status']){
                    echo '已使用';
                }elseif(4 == $order->order->orderCoupon['used_status']){
                    echo '已取消';
                } elseif(0 == $order->order->orderCoupon['used_status']){
                    echo '未激活';
                }
                ?>
            </td>
