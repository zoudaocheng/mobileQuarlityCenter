...
?>
    ...
    <div class="box col-md-12">
        <div class="box-inner">
            <div class="box-header well" data-original-title="">
                <h2><i class="glyphicon glyphicon-edit"></i>项目类型管理</h2>
            </div>
            <div class="box-create">
                <?php
                $form = ActiveForm::begin([
                    'id' => 'list-form',
                    'action' => Url::toRoute('soa/project-type-list'),
                    'options' => ['class' => 'form-horizontal'],
                ]);
                ?>
                <div class="form-group form-group-sm">
                    <div class="col-md-1">
                        <a href="#" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#myDialog" data-data="add"><i class="glyphicon glyphicon-plus-sign"></i><span>添加项目类型</span></a>
                    </div>
                </div>
                <div class="form-group form-group-sm">
    ...
    <div class="modal fade" id="myDialog" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            ...
            $form = ActiveForm::begin([
                'id' => 'edit-form',
                'action' => Url::toRoute('soa/project-type-edit'),
                'options' => ['class' => 'form-horizontal'],
                'fieldConfig' => [
                    'options' => ['class' => NULL],
                ],
    ...
    $(function(){
        ...
        $('#myDialog').on('show.bs.modal', function (event) {
            ...
            if(data=='add')
            {
                ...
                $("#lcbprojecttype-id").val('');
            }
            else{
                modal.find('.modal-title').text("编辑项目类型");
                data=eval(data);
                console.log(data);
                $("#lcbprojecttype-id").val(data.id);
                $("#lcbprojecttype-name").val(data.name);
                $("#lcbprojecttype-description").val(data.description);
            }
        });
